...
describe("It will test the todo routes", () => {
    ...
    test('it should update the todo item', async() => {
        ...
        const item = await request(app).post("/todos").send({title: "title1", description: "wwwww", completed:"true", created_at: "2024-01-11"})
        // const item  = Todo.create({title: "title3", description: "wwwww", completed:true, created_at: "2024-01-13"})

        const response = await request(app).put(`/todos/${item._id}`).send({title: "title2", description: "aaaaa",completed:false,  created_at:"2024-01-13"})
        // expect(response.statusCode).toBe(200)
        expect(response.body.completed).toEqual(false)
        expect(response.body.title).toEqual('title2')
        expect(response.body.title).toEqual('aaaaa')
    })
    ...
    test("it should delete a item", async() => {
        const item5  = Todo.create({title: "title1",description: "wwwww", completed:true, created_at: "2024-01-13"})
